...
const FRAGMENTS = [
  '# ', '## ', '###### ', '> ', '>', '- ', '* ', '+ ', '1. ', '2) ', '- [ ] ', '- [x] ',
  '```', '~~~', '```ts', ':::', '::::', ':::note', ':::note[T]', '::::tabs', ':::tab[A]', ':::chart', ':::unknown',
  '::toc', '::progress{value=5}', ':badge[b]{variant=info}', ':kbd[K]', ':x[', ':y{', '::: spaced',
  '*', '**', '_', '__', '~~', '`', '``', '[', ']', '(', ')', '![', '{', '}', '#', '.', '=', '"', "'",
  '[a](b)', '[ref]', '[ref]: /u', '[^1]', '[^1]: note', '<!--', '-->', '<https://x.y>', 'https://a.b/c',
  '|', '| a | b |', '|---|---|', '---', '***', '===', '\\', '&amp;', '&#65;', '&bogus;',
  'text', 'word', ' ', '  ', '\t', 'é', '😀', '\u0000', 'type: bar', 'data:', '  a: 1', 'labels: [a', '- name: s',
];

const line = fc.array(fc.constantFrom(...FRAGMENTS), { maxLength: 8 }).map((parts) => parts.join(''));
...
describe('parser properties', () => {
  ...
  it('reports the same diagnostic codes regardless of line endings', () => {
    const lfDocument = fc.array(fc.tuple(indent, line), { maxLength: 20 }).map((lines) => lines.map(([i, l]) => i + l).join('\n'));
    fc.assert(
      fc.property(lfDocument, (source) => {
        const codes = (s: string) => parse(s).diagnostics.map((d) => `${d.code}@${d.range.start.line}:${d.range.start.column}`);
