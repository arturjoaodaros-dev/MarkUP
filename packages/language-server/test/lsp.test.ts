...
const diagnostics = new Map<string, { code: string; message: string; range: { start: { line: number; character: number } } }[]>();
const waiters: (() => void)[] = [];

beforeAll(async () => {
  workspace = mkdtempSync(join(tmpdir(), 'markup-lsp-'));
  writeFileSync(join(workspace, 'shout.mjs'), `export default { name: 'shout', directives: [{ name: 'shout', forms: ['inline'], description: 'Shout.' }] };`);
  writeFileSync(join(workspace, 'markup.config.json'), JSON.stringify({ plugins: ['./shout.mjs'] }));
  child = spawn(process.execPath, ['--conditions=source', bin, '--stdio'], { stdio: ['pipe', 'pipe', 'inherit'] });
  connection = createMessageConnection(new StreamMessageReader(child.stdout!), new StreamMessageWriter(child.stdin!));
  connection.onNotification('textDocument/publishDiagnostics', (params: { uri: string; diagnostics: never[] }) => {
...
async function open(text: string, version = 1): Promise<void> {
  ...
  await connection.sendNotification('textDocument/didOpen', { textDocument: { uri, languageId: 'markup', version, text } });
  await received;
}
...
describe('language server', () => {
  it('publishes diagnostics on open and on change', async () => {
    ...
    await connection.sendNotification('textDocument/didChange', {
      ...
      contentChanges: [{ range: { start: { line: 2, character: 3 }, end: { line: 2, character: 7 } }, text: 'note' }],
    });
    await received;
  ...
  it('completes components with snippet edits', async () => {
    await open('Text\n\n:::ca', 3);
    const items = await connection.sendRequest<{ label: string; insertTextFormat: number; textEdit: { newText: string; range: { start: { character: number } } }; filterText: string }[]>('textDocument/completion', {
      textDocument: { uri },
  ...
  it('answers hover, symbols, folding, semantic tokens and definitions', async () => {
    await open('# Intro\n\n:::chart\ndata:\n  a: 1\n:::\n\n[up](#intro)', 4);
    const hover = await connection.sendRequest<{ contents: { value: string } }>('textDocument/hover', { textDocument: { uri }, position: { line: 2, character: 4 } });
    expect(hover.contents.value).toContain('**chart**');
    const symbols = await connection.sendRequest<{ name: string; children: { name: string }[] }[]>('textDocument/documentSymbol', { textDocument: { uri } });
    expect(symbols[0]!.name).toBe('Intro');
    expect(symbols[0]!.children[0]!.name).toBe('chart');
    const folds = await connection.sendRequest<{ startLine: number; endLine: number }[]>('textDocument/foldingRange', { textDocument: { uri } });
    ...
    const definition = await connection.sendRequest<{ range: { start: { line: number } } }>('textDocument/definition', { textDocument: { uri }, position: { line: 7, character: 2 } });
    expect(definition.range.start.line).toBe(0);
  });

  it('offers quick fixes', async () => {
    await open(':::nott\nx\n:::', 5);
    const actions = await connection.sendRequest<{ title: string; edit: { changes: Record<string, { newText: string }[]> } }[]>('textDocument/codeAction', {
      textDocument: { uri },
